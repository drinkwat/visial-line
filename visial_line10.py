...
def create_csv(filename):
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        # 3. 写入表头
        writer.writerow(['ID', 'differ', 'grdient', 'line_speed','angular_speed'])
...
class LaneLines:
    ...
    def measure(self, lane=2):
        ...
        if lane == 0:  # 左线
            ...
            differ = -((xr_b_pix + xl_b_pix)/2 -xl - 119) - 6.95 -0.78
            print(f"L-differ = {differ}")
            
        elif lane == 1:  # 右线

            xr = np.dot(self.right_fit, [transform_dst_size[1], 1])
            differ = xr - (xr_b_pix + xl_b_pix)/2 - 108 - 2.1 + 7.2
    ...
    while flag_task_l.value:
        ...
        if flag_lane_l.value:
            ...
            try:
                ...
                # 创建一个包含5个元素的空列表（初值为 None）
                
                data_list[1] = differ
                data_list[2] = gradiant
                data_list[3] = min(round(max_vx - 0.2 + kp_x / abs(gradiant), 2), max_vx) * vel_2_0x40
                data_list[4] = kp_w * differ * omega_2_0x40
