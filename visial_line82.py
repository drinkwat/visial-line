...
class LaneLines:
    ...
    def find_lane_pixels(self, img, lane=2, image=False, debug=False):
        ...
        if lane == 0:
            ...
            if debug:
                cv2.waitKey(0)
            return left_x, left_y,left_ang,left_ang_sum,left_zero
        # 寻找右线
        elif lane == 1:
            # 找到直方图右半区域中的第一个峰
            ...
            if debug:
                cv2.waitKey(0)
            return right_x, right_y,right_ang,right_ang_sum,right_zero
        # 两条线都寻找
        else:
            left_x_base = np.argmax(histogram[:self.midpoint])
    ...
    def fit_poly(self, img, image=False, debug=False):
        ...
        left_x, left_y,left_ang,left_ang_sum,left_zero = self.find_lane_pixels(img, lane=0, image=image, debug=debug)
        right_x, right_y,right_ang,right_ang_sum,right_zero = self.find_lane_pixels(img, lane=1, image=image)
        
        print(f"left_zero = {left_zero}")
        print(f"right_zero = {right_zero}")
        if left_ang >= 1 and right_ang >= 1 and left_zero <= 2 and right_zero <= 2:
            final_stop = 1
            my_print("到达停止标志")
