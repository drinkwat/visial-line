...
class LaneLines:
    ...
    def find_lane_pixels(self, img, lane=2, image=False, debug=False):
        ...
        img_R[:, :150] = 0
        # 创建一张用来后续做可视化的三通道图片
        out_img_L = np.dstack((img_l, img_l, img_l))
        out_img_R = np.dstack((img_R, img_R, img_R))
        ...
        if lane == 0:
            ...
            cv2.imshow('out_img_L', out_img_L)

            # 创建输入的二值图底部一个滑窗高度的区域的直方图
            histogram = np.sum(img_l[img_l.shape[0] // 4:, :], axis=0)  # img[h, w, channel]

            # 找到直方图中左起第一个峰，认为是左线的起点
            left_x_base = np.argmax(histogram[:self.midpoint])

            # my_print(f"self.midpoint = {self.midpoint}")
            ...
            # 可视化
            # if image:
            out_img_L[left_y, left_x] = [255, 0, 0]
            cv2.line(out_img_L, (xl_b_pix, self.img_size[1] - 10), (xl_b_pix, self.img_size[1]), (255, 255, 255), 5)
            cv2.line(out_img_L, (xr_b_pix, self.img_size[1] - 10), (xr_b_pix, self.img_size[1]), (255, 255, 255), 5)
            cv2.imshow('slide windows L', out_img_L)
            cv2.moveWindow("slide windows L", 25, 350)
            if debug:
                cv2.waitKey(0)
            return left_x, left_y,left_ang,left_ang_sum
        # 寻找右线
        elif lane == 1:
            
            cv2.imshow('out_img_R', out_img_R)
             # 创建输入的二值图底部一个滑窗高度的区域的直方图
            histogram = np.sum(img_R[img_R.shape[0] // 4:, :], axis=0)  # img[h, w, channel]
            # 找到直方图右半区域中的第一个峰
            right_x_base = np.argmax(histogram[self.midpoint+40:]) + self.midpoint+40
            my_print(f"right_x_base = {right_x_base}")
            ...
            # if image:
            out_img_R[right_y, right_x] = [0, 0, 255]
            cv2.line(out_img_R, (xl_b_pix, self.img_size[1] - 10), (xl_b_pix, self.img_size[1]), (255, 255, 255), 5)
            cv2.line(out_img_R, (xr_b_pix, self.img_size[1] - 10), (xr_b_pix, self.img_size[1]), (255, 255, 255), 5)
            cv2.imshow('slide windows R', out_img_R)
            cv2.moveWindow("slide windows R", 25, 350)
            if debug:
                cv2.waitKey(0)
            return right_x, right_y,right_ang,right_ang_sum
        # 两条线都寻找
        else:
            ...
            for i in range(self.n_windows):
                ...
                if len(good_left_x) > self.min_pix:
                    ...
                    print(f"当前左线线第{i}个窗口有{len(good_left_x)}个车道线点")
                if len(good_right_x) > self.min_pix:
                    right_x_current = np.int32(np.mean(good_right_x))
                    print(f"当前右线线第{i}个窗口有{len(good_right_x)}个车道线点")
    ...
    def fit_poly(self, img, image=False, debug=False):
        ...
        left_x, left_y,left_ang,left_ang_sum = self.find_lane_pixels(img, lane=0, image=image, debug=debug)
        right_x, right_y,right_ang,right_ang_sum = self.find_lane_pixels(img, lane=1, image=image)
