...
class LaneLines:
    ...
    def calculate(self):
        ...
        print(f"abs(gradiant_l){abs(gradiant_l)}")
        print(f"abs(gradiant_r){abs(gradiant_r)}")

        print(f"(abs(gradiant_l)+abs(gradiant_r)) = {(abs(gradiant_l)+abs(gradiant_r))}")
        print(f"abs(abs(gradiant_l) - abs(gradiant_r)) = {abs(abs(gradiant_l) - abs(gradiant_r))}")
        
        if right_angle_1 == 0:
            if left_exit and right_exit and (abs(gradiant_l)+abs(gradiant_r)) >0.3 and abs(abs(gradiant_l) - abs(gradiant_r))>0.3 :
                print("A")
        ...
        if right_angle_1 == 1 and (abs(gradiant_l)+abs(gradiant_r))> 0.7 and abs(abs(gradiant_l) - abs(gradiant_r))<0.7:
            right_angle_2 = 1
            print("执行转弯")
        
        print(f"right_angle_2 = {right_angle_2}")

        # print(f"turn_left = {turn_left}")
        print("****************************************************************")
...
def chassis_control(flag_task_c, cmd_chassis_c):
    ...
    while flag_task_c.value:
        ...
        elif cmd_chassis_c[0] == turn_left_l or turn_right_l:
