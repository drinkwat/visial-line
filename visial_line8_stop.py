...
class LaneLines:
    ...
    def find_lane_pixels(self, img, lane=2, image=False, debug=False):
        ...
        if lane == 0:
            ...
            if debug:
                cv2.waitKey(0)
            return left_x, left_y,left_ang,left_ang_sum,left_kuang
        # 寻找右线
        elif lane == 1:
            # 找到直方图右半区域中的第一个峰
            ...
            if debug:
                cv2.waitKey(0)
            return right_x, right_y,right_ang,right_ang_sum,right_kuang
        # 两条线都寻找
        else:
            left_x_base = np.argmax(histogram[:self.midpoint])
    ...
    def fit_poly(self, img, image=False, debug=False):
        ...
        left_x, left_y,left_ang,left_ang_sum,left_kuang = self.find_lane_pixels(img, lane=0, image=image, debug=debug)
        right_x, right_y,right_ang,right_ang_sum,right_kuang = self.find_lane_pixels(img, lane=1, image=image)
        ...
        # 选择巡线
        print(f"left_exit = {left_exit}")
        print(f"right_exit = {right_exit}")
        print(f"len(left_y) = {len(left_y)}")
        print(f"len(right_y) = {len(right_y)}")
        ...
        print("                              ")
        print(f"left_kuang = {left_kuang}")
        print(f"right_kuang = {right_kuang}")
        if left_ang >= 1 and right_ang >= 1 and left_kuang<=4 and right_kuang<=4 and left_exit==1 and right_exit==1:
            final_stop = 1
            print("已到达终点")
