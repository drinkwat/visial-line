...
增加计数，停车后前行一小段距离，然后停车
标志位1 在直角弯前判断更新，在转弯后清零，重新判断
不会自动更新为0
如果没有直角弯,会一直强制切线（切左切线要更新）
仅能跑简单直角弯,绝对不能跑S弯
标志位2在1秒内不会重复置1
切线纠正
"""
...
class LaneLines:
    ...
    def find_lane_pixels(self, img, lane=2, image=False, debug=False):
        ...
        if lane == 0:
            ...
            if debug:
                cv2.waitKey(0)
            return left_x, left_y,left_ang,left_ang_sum,left_zero,left_line,left_line_posi,left_ang_posi
        # 寻找右线
        elif lane == 1:
            # 找到直方图右半区域中的第一个峰
            ...
            if debug:
                cv2.waitKey(0)
            return right_x, right_y,right_ang,right_ang_sum,right_zero,right_line,right_line_posi,right_ang_posi
        # 两条线都寻找
        else:
            left_x_base = np.argmax(histogram[:self.midpoint])
    ...
    def fit_poly(self, img, image=False, debug=False):
        ...
        left_x, left_y,left_ang,left_ang_sum,left_zero,left_line,left_line_posi,left_ang_posi = self.find_lane_pixels(img, lane=0, image=image, debug=debug)
        right_x, right_y,right_ang,right_ang_sum,right_zero,right_line,right_line_posi,right_ang_posi = self.find_lane_pixels(img, lane=1, image=image)
        ...
        if right_line == 1 and right_line_posi == 8 and (left_line_posi > 0 and left_line_posi <= 6) and (left_line_posi + left_zero) == 8 and left_line_posi != right_line_posi :
            turn_left = 0
        ...
        elif left_line == 1 and left_line_posi == 8 and (right_line_posi > 0 and right_line_posi <= 6) and (right_line_posi + right_zero) == 8 and left_line_posi != right_line_posi :
            turn_left = 0
            right_angle_1 = 1 
            print("到达右转第一标志位")
        ...
        if right_angle_1 ==1 or right_angle_1 == 0:
            if left_ang >= 1 and right_ang >= 1 and left_line_posi != right_line_posi:
                ...
                my_print("到达第二标志位")
                if turn_left:
                    my_print(right_ang_posi)
                else:
                    my_print(left_ang_posi)
    ...
    def measure(self, lane=2):
        ...
        if lane == 0:  # 左线
            ...
            differ = -((xr_b_pix + xl_b_pix)/2 -xl - 119 -2.5) -6.3 -3 +4 - 2.5
            print(f"L-differ = {differ}")
            
        elif lane == 1:  # 右线

            xr = np.dot(self.right_fit, [transform_dst_size[1], 1])
            differ = xr - (xr_b_pix + xl_b_pix)/2 - 108 +1.6 -1 - 1.2 + 6 + 0.5
            print(f"R-differ = {differ}")
